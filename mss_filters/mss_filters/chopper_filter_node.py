...
class ChopperFilterNode(Node):
    def __init__(self):
        ...
        self.declare_parameter('spike_threshold_heading', 30.0)  # Próg wykrywania szpilek w headingu (stopnie)
        self.declare_parameter('spike_min_speed', 0.0)  # Minimalna fizycznie możliwa prędkość (m/s)
        self.declare_parameter('spike_max_speed', 50.0)  # Maksymalna fizycznie możliwa prędkość (m/s)
        ...
        self.spike_filter_service = self.create_service(
            SetBool,
            '/mss/filters/chopper/spike_filter_toggle',
    ...
    def listener_callback(self, msg):
        ...
        if self.median_filter_enabled:
            self.get_logger().debug(f'Przefiltrowane dane sieczkarni: Speed={filtered_speed[0]:.2f} m/s (median: {median_speed:.2f}), Lat={filtered_lat[0]:.8f}, Lon={filtered_lon[0]:.8f}, Heading={self.normalize_heading(filtered_heading[0]):.2f}° (median: {median_heading:.2f}°){spike_info}')
        else:
            self.get_logger().debug(f'Przefiltrowane dane sieczkarni: Speed={filtered_speed[0]:.2f} m/s (raw: {msg.speed_mps:.2f}, median OFF), Lat={filtered_lat[0]:.8f}, Lon={filtered_lon[0]:.8f}, Heading={self.normalize_heading(filtered_heading[0]):.2f}° (raw: {msg.heading_deg:.2f}°, median OFF){spike_info}')
