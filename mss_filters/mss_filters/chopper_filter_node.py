...
class ChopperFilterNode(Node):
    ...
    def listener_callback(self, msg):
        ...
        self.get_logger().debug(f'>>> Otrzymano dane GPS sieczkarni! Prędkość surowa: {msg.speed_mps:.2f} m/s, Lat: {msg.latitude_deg:.8f}, Lon: {msg.longitude_deg:.8f}')
        ...
        if self.median_filter_enabled:
            self.get_logger().debug(f'Przefiltrowane dane sieczkarni: Speed={filtered_speed[0]:.2f} m/s (median: {median_speed:.2f}), Lat={filtered_lat[0]:.8f}, Lon={filtered_lon[0]:.8f}')
        else:
            self.get_logger().debug(f'Przefiltrowane dane sieczkarni: Speed={filtered_speed[0]:.2f} m/s (raw: {msg.speed_mps:.2f}, median OFF), Lat={filtered_lat[0]:.8f}, Lon={filtered_lon[0]:.8f}')
